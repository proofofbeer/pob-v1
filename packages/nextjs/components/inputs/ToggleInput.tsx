...
const ToggleInput = ({ name, onChange, isDefaultValueA, valueA, valueB }: TToggleInputProps) => {
  ...
  return (
    <div className="flex items-center h-[2.2rem] min-h-[2.2rem] space-x-2 mt-2 p-4 text-lg pointer-events-auto">
      <span className={`${isActiveValueA ? "text-orange-600" : ""} mr-2 font-medium`}>{valueA}</span>
      <input
        id={name}
        name={name}
        type="checkbox"
      ...
      />
      <span className={`${!isActiveValueA ? "text-orange-600" : ""} ml-2 font-medium`}>{valueB}</span>
    </div>
  );
};
