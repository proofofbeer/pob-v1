...
uploadApi.post(async (req, res) => {
  ...
  const imagesCid = await nftStorage.storeDirectory(
    files.map((file: any, index: number) => new File([file.buffer], `image-${index}`)),
  );
  const imageStatus = await nftStorage.status(imagesCid);

  const cid = await nftStorage.storeDirectory(
    nfts.map(
      (nft: any, index: number) => new File([JSON.stringify(erc1155Metadata(nft, imagesCid, index))], `nft-${index}`),
    ),
  );
  const status = await nftStorage.status(cid);

  res.json({ imagesCid, imageStatus, cid, status });
});
